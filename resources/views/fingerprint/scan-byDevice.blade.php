...
<div class="card card-secondary">
    ...
    <form method="POST" action="{{ $actionUrl }}" id="fingerprintForm">
        ...
        <div class="card-footer d-flex gap-2">
            ...
            @isset($student->fingerprint_id)
                <form action="{{ route('fingerprint.clear', $student->id) }}" method="POST" onsubmit="return confirm('Yakin mau clear fingerprint?')">
                    @csrf
                    @method('POST')
...
<script>
    ...
    if(studentId){
        ...
        // Set waiting ketika halaman dibuka
        fetch('/fingerprint/wait/' + studentId, {
            method: 'POST',
            headers: {
