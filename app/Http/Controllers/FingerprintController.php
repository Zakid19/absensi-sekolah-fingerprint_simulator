...
class FingerprintController extends Controller
{
    ...
    public function student_reset()
    {
        ...
        return view('fingerprints.classes', compact('classes'));
    }

    public function showClass($id)
    {
        $class = ClassRoom::with('students')->findOrFail($id);
        return view('fingerprints.reset', compact('class'));
    }

    public function reset(Student $student)
    {
        // 1. hapus fingerprint dari siswa
        $oldFingerprint = $student->fingerprint_id;
        $student->update([
            'fingerprint_id' => null,
        ]);

        // 2. bersihkan pending fingerprint lama (jaga konsistensi)
        if ($oldFingerprint) {
            PendingFingerprint::where('fingerprint_id', $oldFingerprint)
                ->delete();
        }

        // 3. otomatis masuk MODE REGISTRASI
        SystemSetting::set('fingerprint_mode', 'register');
